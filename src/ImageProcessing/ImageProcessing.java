...
public class ImageProcessing {
    ...
    public static ImageProcessingResult processImage(Mat image) {
		...
		/*
		 * // Draw the best circle found on the original image if (bestCenter != null) {
		 * Imgproc.circle(image, bestCenter, (int) bestRadius, new Scalar(0, 255, 0),
		 * 3); Imgproc.rectangle( image, new Point(bestCenter.x - bestRadius,
		 * bestCenter.y - bestRadius), new Point(bestCenter.x + bestRadius, bestCenter.y
		 * + bestRadius), new Scalar(255, 0, 0), 2 ); //
		 * System.out.println("Most circular circle found: Center = " + bestCenter +
		 * ", Radius = " + bestRadius); } else {
